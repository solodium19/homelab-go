...
В окружении Stage уже не так, тут мы воссоздаём среду для тестирования и предпродового состояния, в котором оценим реальную нагрузку приложения, посмотрим как оно себя ведёт в разных состояниях, сделаем небольшое скалирование HPA или Keda, ограничим ресурсы, возможно воспользуемся VPA, вообщем приведем к предпродовому состоянию, а вот уже в окружении Prod сделаем конечные манифесты

## Добавление helm
Helm чарты для разных окружений написаны, прописаны в ArgoCD и Gitlab CI, поэтому сейчас тестовые деплои происходят с помощью helm, сам чарт можно посмотреть в директории chart-homelab. Написан один общий чарт, а для разных окружений расписаны разные values.yaml файлы
